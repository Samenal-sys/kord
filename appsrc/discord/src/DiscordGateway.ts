...
export default class Gateway extends EventEmitter<GatewayEventsMap> {
    ...
    init() {
        ...
        import("./lib/wrapped").then(async ({ createInflateInstance }) => {
            ...
            // Allow WebSocket to handle both strings (uncompressed) and arraybuffers (compressed)
            ws.addEventListener("message", async ({ data }: MessageEvent) => {
                // If uncompressed JSON string, bypass pako worker completely
                if (typeof data === "string") {
                    try {
                        this.#handlePacket(JSON.parse(data));
                    } catch (e) {
                        this.logger.err("Failed to parse JSON packet", e)();
                    }
                    return;
                }

                if (!(data instanceof ArrayBuffer)) {
                    this.logger.err("Received unknown data format!", data)();
                    return;
                }
