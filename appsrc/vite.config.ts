...
const isKai3 = ["0", "3"].includes(process.env.KAIOS);
const isCloudphone = process.env.CLOUDPHONE == "1";
const production = process.env.NODE_ENV === "production";
const isKord = process.env.KAIOS == "0"
...
export default defineConfig({
	plugins: [
		...
		(isKai3 || isKord) && {
    name: "fix-html-module-type",
    transformIndexHtml(html: string) {
        return html.replace(/type="systemjs-module"/g, 'type="module"');
    }
},
		patchCssModules({
			exportMode: "named",
		...
		(!isCloudphone && !isKord) &&
			zipPack({
				inDir: (() => {
  			if (isKord) return "kord";
  			if (isKai3) return "dist-v3";
  			return "dist";
		})(),
				outDir: "builds",
	...
	build: {
		...
		cssMinify: isDevBuild ? false : true,
		ssr: false,
		sourcemap: isDevBuild,

		rollupOptions: {
			output: {
				format: (isKai3 || isKord) ? "esm" : "systemjs",
